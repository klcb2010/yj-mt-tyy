...
def main(username, password):
    ...
    try:
        loginhash = re.findall('loginhash=(.*?)">', chusihua.text)
        formhash = re.findall('formhash" value="(.*?)".*? />', chusihua.text)
    except Exception:
        return False
    ...
    if '欢迎您回来' in denlu:
        fzmz = re.findall('欢迎您回来，(.*?)，现在', denlu)
        ...
        formhash = re.findall('formhash" value="(.*?)".*? />', zbqd)
